...
_rein() {
    ...
    if [[ $COMP_CWORD -eq 1 ]]; then
        COMPREPLY=( $(compgen -W "in spec list mcp configure undo gateway update version help completion" -- "$cur")
                    $(compgen -c -- "$cur") )
        return
    fi
    case ${COMP_WORDS[1]} in
        ...
        undo)
            COMPREPLY=( $(compgen -W "claude-code codex" -- "$cur") ) ;;
        gateway)
...
_rein() {
    if (( CURRENT == 2 )); then
        _alternative \
            'verbs:rein command:(in spec list mcp configure undo gateway update version help completion)' \
    ...
    case $words[2] in
        ...
        undo)
            _values undo claude-code codex ;;
        gateway)
...
complete -c rein -f
complete -c rein -n "__fish_use_subcommand" -a "in spec list mcp configure undo gateway update version help completion" -d "rein command"
...
complete -c rein -n "__fish_seen_subcommand_from undo" -a "claude-code codex"
