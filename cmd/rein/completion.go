...
_rein() {
    ...
    if [[ $COMP_CWORD -eq 1 ]]; then
        COMPREPLY=( $(compgen -W "in spec list mcp update version help completion" -- "$cur")
                    $(compgen -c -- "$cur") )
        return
    fi
...
_rein() {
    if (( CURRENT == 2 )); then
        _alternative \
            'verbs:rein command:(in spec list mcp update version help completion)' \
...
complete -c rein -f
complete -c rein -n "__fish_use_subcommand" -a "in spec list mcp update version help completion" -d "rein command"
